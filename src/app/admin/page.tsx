...
export default async function AdminDashboard() {
  ...
  const stats = [
    { name: 'Total Properties', value: data.propertyCount, icon: Home, color: 'bg-blue-500' },
    { name: 'Total Blogs', value: data.blogCount, icon: FileText, color: 'bg-green-500' },
    { name: 'New Inquiries', value: data.inquiryCount, icon: MessageSquare, color: 'bg-purple-500' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            ...
            <Link href="/admin/inquiries" className="text-sm text-blue-600 hover:underline">View All</Link>
          </div>
          <div className="p-6">
              ...
              <ul className="divide-y divide-gray-50">
                {data.recentInquiries.map((inquiry: any) => (
                  <li key={inquiry._id} className="py-3">
                    ...
                    <p className="text-xs text-gray-500">{inquiry.email} • {new Date(inquiry.createdAt).toLocaleDateString()}</p>
                  </li>
                ))}
              </ul>
            )}
            ...
            {data.recentProperties.length === 0 ? (
              ...
              <ul className="divide-y divide-gray-50">
                {data.recentProperties.map((property: any) => (
                  <li key={property._id} className="py-3 flex justify-between items-center">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{property.title}</p>
                      <p className="text-xs text-gray-500">{property.location.area} • ₹{property.price.toLocaleString('en-IN')}</p>
                    </div>
